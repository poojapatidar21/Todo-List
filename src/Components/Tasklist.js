...
function Tasklist() {
  ...
  const [completedtasks, setcompletedtasks] = useState([]);
    let arr = [];
    useEffect(() => {
        db.collection('tasks').where('isCompleted','==',false).get().then((querySnapshot) => {
            let a = [];
            let c = 0;
  ...
  useEffect(() => {
      db.collection('tasks').where('isCompleted','==',true).get().then((querySnapshot) => {
          let a = [];
          let c = 0;
          querySnapshot.forEach((doc) => {
              a.push({ ...doc.data(), id: doc.id });
  ...
  const list = tasks.map((i, index) => (
    <li className={`id${i.id}`} onClick={() => strikeout(i.id, index, i.name,i.isCompleted)}>
      {i.name}
    </li>
  ));

  const list2 = completedtasks.map((i, index) => (
    <li >
      {i.name}
    </li>
  ...
  return (
    <div className="containers">
      ...
      <div className="todolist">
        ...
        <Button variant="primary" className="button" onClick={makeCompleted}>Remove Completed Task</Button>
        </div>
        }
      </div>
      {list2.length > 0 &&
